...
What this module gives the pipeline:
- BRAND: name / tagline / host / palette / content formula / AI boundary.
- DNA palettes: each script-DNA segment (場景/衝突/洞察/方法/反思) maps to a
  (color1, color2, name) tuple drawn from the brand palette, so the
  free gradient renderer auto-styles each beat of the story.
- SERIES: the 10 product lines + 壽司切片 short-form.
- SEED_TOPICS: the 6 first-quarter 母題, each with the host's 原創判斷.
- parse_dna(): turn a marker-delimited script into one shot per beat.
"""
...
DNA_MARKERS = list(DNA_PALETTES.keys())
...
def parse_dna(script: str):
    ...
    The parser module turns the returned tuples into Shot objects, so this
    only needs to yield (label, text) pairs in story order.
    """
    import re

    # Accept full-width or half-width brackets, with or without the 】 close.
    pattern = re.compile(
