...
## 7. 測試 / Testing
- ✅ **pytest 27 測試涵蓋 config/parser/tts/renderer/db/api/ci/security/integration/runway/openai**（CI 綠燈）。
...
---
下一步建議優先序：⑥ 剩餘項（失敗 video_url 前端處理 / 可視化指標）→ ① 按 shot.index 顯式排序 → ⑤ Docker Hub/真 Runway 實測（需密鑰）。
